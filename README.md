...
3. Перейти по адресу: `http://localhost:8080/swagger/index.html` 
5. Отправлять запросы(При указании нового ClientId - создаётся новый пользователь; Даты указаны в UTC для последующей работы с разными часовыми поясами)
P.S. Nginx сам решает на какой экземпляр API придёт запрос.
