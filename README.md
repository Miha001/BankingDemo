...
1. В папке ***BankingDemo.API*** с выполнить команду `docker compose up --build`(Через .yml файл запустятся БД и 2 API, накатятся миграции)
2. Дождаться логов запуска в терминале.
3. Для использования Api-1 перейти по адресу: `http://localhost:8081/swagger/index.html`
4. Для использования Api-2 перейти по адресу: `http://localhost:8082/swagger/index.html`
5. Юзать запросы(При указании нового ClientId - создаётся новый пользователь; Даты указаны в UTC для последующей работы с разными часовыми поясами)
